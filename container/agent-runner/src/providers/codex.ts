...
function composeBaseInstructions(input: QueryInput): string | undefined {
  const pieces: string[] = [];

  const bundle = input.systemContext?.promptBundle;
  if (bundle) {
    if (bundle.globalInstructions) pieces.push(bundle.globalInstructions);
    if (bundle.groupInstructions) pieces.push(bundle.groupInstructions);
  }
    ...
    async function* gen(): AsyncGenerator<ProviderEvent> {
      ...
      try {
        ...
        const threadParams = {
          ...
          baseInstructions: composeBaseInstructions(input),
        };
