...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash pipipiclaw-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash pipipiclaw-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash pipipiclaw-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash pipipiclaw-agent:latest
```
...
echo '{}' | docker run -i --entrypoint /bin/echo pipipiclaw-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
