...
<dict>
    ...
    <string>${projectRoot}/logs/pipipiclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/pipipiclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.pipipiclaw');
  } catch {
...
/**
 * Kill any orphaned pipipiclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned pipipiclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/pipipiclaw.service';
    ...
    unitPath = path.join(unitDir, 'pipipiclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=Pipipiclaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/pipipiclaw.log
StandardError=append:${projectRoot}/logs/pipipiclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned pipipiclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable pipipiclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start pipipiclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active pipipiclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-pipipiclaw.sh');
  const pidFile = path.join(projectRoot, 'pipipiclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-pipipiclaw.sh — Start Pipipiclaw without systemd',
    ...
    '    echo "Stopping existing Pipipiclaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting Pipipiclaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/pipipiclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/pipipiclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "Pipipiclaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/pipipiclaw.log"`,
  ];
